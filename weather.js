...
var newLi = $("<li>");
newLi.html(cityArray[i]);
$(".history").append(newLi)
}

$(".history").on("click",function(e){
    var savedCity = e.target.innerHTML;
    fetch('https://api.openweathermap.org/data/2.5/forecast?q=' + savedCity + '&appid=6dffc89cea0871519ae20ba624a3e047&units=imperial')
    .then(response => response.json())
    .then(data => {
        var cityLat = data.city.coord.lat;
        var cityLon = data.city.coord.lon;
        fetch('https://api.openweathermap.org/data/2.5/onecall?lat='+ cityLat + "&lon=" + cityLon + '&appid=6dffc89cea0871519ae20ba624a3e047')
        .then(response => response.json())
        .then(data => $("#UV-index ").html("UV Index: "+ data.current.uvi));
